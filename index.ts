...
async function main() {
  ...
  if (captures.length > 0) {
    ...
    if (combinedHtml.length > 100) {
      // Check if design.md and extracted-components.html already exist
      const designExists = await Bun.file(join(".data", siteName!, "design.md")).exists();
      const componentsExist = await Bun.file(join(".data", siteName!, "extracted-components.html")).exists();

      if (designExists && componentsExist) {
        success("design.md and extracted-components.html already exist.");
        info("To regenerate, delete them and re-run.");
      } else {
        // Ask which mode to use
        console.log("");
        const modeChoice = await select("How would you like to generate design.md?", [
          "Auto (fast, basic — generated by the CLI)",
          "Agent (recommended — use instructions.md with an AI agent)",
          "Skip (don't generate now)",
        ]);

        if (modeChoice === 0) {
          // Auto mode
          info("Generating with auto mode...");

          if (!designExists) {
            info("Extracting design tokens...");
            const designMd = generateDesignMd(combinedHtml, siteName!);
            await Bun.write(join(".data", siteName!, "design.md"), designMd);
            success(`design.md (${(designMd.length / 1024).toFixed(0)}KB)`);
          } else {
            info("design.md already exists, skipping.");
          }

          if (!componentsExist) {
            info("Extracting components...");
            const componentsHtml = extractComponents(renderedPaths, renderedContents, siteName!);
            await Bun.write(join(".data", siteName!, "extracted-components.html"), componentsHtml);
            success(`extracted-components.html (${(componentsHtml.length / 1024).toFixed(0)}KB)`);
          } else {
            info("extracted-components.html already exists, skipping.");
          }
        } else if (modeChoice === 1) {
          // Agent mode — copy instructions.md into the site's data folder
          const srcInstructions = join("instructions.md");
          const destInstructions = join(".data", siteName!, "instructions.md");
          try {
            const instructionsContent = await Bun.file(srcInstructions).text();
            await Bun.write(destInstructions, instructionsContent);
            success(`instructions.md copied to .data/${siteName!}/`);
          } catch {
            warn("instructions.md not found in project root.");
          }

          info("");
          info("Agent mode selected. To generate design.md:");
          info("");
          info("  1. Open these files:");
          info(`     .data/${siteName!}/instructions.md`);
          for (const f of renderedPaths) {
            info(`     .data/${siteName!}/${f}`);
          }
          info("  2. Feed them to any AI agent (Claude, GPT, Gemini, etc.)");
          info("  3. The agent will produce design.md + extracted-components.html");
          info("  4. Save the outputs to .data/" + siteName! + "/");
          info("");
          info("Example prompt:");
          info('  "Analyze this rendered HTML and generate design.md +');
          info('   extracted-components.html following the instructions."');
        } else {
          info("Skipped design system generation.");
        }
      }
    } else {
